...
where
    ...
    T: Transport + Clone,
{
    const BATCH_SIZE: usize = 50; // 限制每批最多50个地址
    
    println!("🔍 multicall_balance_of: Total addresses to query: {}", addresses.len());
    ...
    for (batch_idx, chunk) in addresses.chunks(BATCH_SIZE).enumerate() {
        println!("🔍 multicall_balance_of: Processing batch {} with {} addresses", batch_idx + 1, chunk.len());
        ...
        let multicall_instance = Multicall3::new(MULTICALL_CONTRACT_ADDRESS, provider.clone());
        ...
        println!("🔍 multicall_balance_of: Batch {} returned {} balances", batch_idx + 1, result.len());
        all_balances.extend(result);
    }
    
    println!("🔍 multicall_balance_of: Total balances collected: {}", all_balances.len());
    Ok(all_balances)
}

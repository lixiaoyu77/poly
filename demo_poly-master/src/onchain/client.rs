...
sol! {
    ...
    contract BatchTransfer {
        function batchTransferERC20(address token, uint256 total, address[] recipients, uint256[] amounts) external;
        function batchTransferETH(address[] recipients, uint256[] amounts) external payable;
    }
}
    ...
    T: Transport + Clone,
{
    ...
    pub async fn approve(
        ...
        // Only meaningful for ERC20
        if !token.is_erc20 {
            return Ok(String::new());
        }
        let contract_instance = IERC20::new(token.contract_address, self.provider.clone());
        let input = IERC20::approveCall { spender, amount: value }.abi_encode();
        let tx_hash = self
            .send_transaction(token.contract_address, Some(input.into()), U256::from(0))
            .await?;
        Ok(tx_hash)
    }

    pub async fn batch_transfer_erc20(
        &self,
        ...
        let input = BatchTransfer::batchTransferERC20Call {
        ...
        let tx_hash = self
            .send_transaction(batch_contract, Some(input.into()), U256::from(0))
            .await?;
        Ok(tx_hash)
    }
    ...
    pub async fn send_transaction(
        ...
        let txh = format!("0x{:x}", receipt.transaction_hash);
        if tx_status {
            tracing::info!(
            ...
            tracing::error!("Transaction failed: {url}/tx/{}", txh);
        }

        Ok(txh)
    }

    pub async fn transfer(&self, to: Address, value: U256, token: &Token) -> eyre::Result<String> {
        let tx_hash = match token.is_erc20 {
            true => {
